...
D_vals = linspace(-2*D_central,2*D_central,nMesh);
...
for dd=1:length(D_vals) % loop over each D
    E_vals = linspace(0,D_vals(dd)/3,idivide(dd,int16(3))); % want same amount of separation for each value of D
    ...
    for ee=1:length(E_vals) % loop over each E
        ...
        parfor ii = 1:length(distance_range)
            ...
            Sys = struct('g',[2.00231930436153,1.992],'S',[7/2,1/2],...
                'eeD',current_coupling,'D',[0,0,0; ZFS]);
            [~, pake] = pepper(Sys, Exp);
            out(:, ii) = pake/sum(pake); % normalize
        end
